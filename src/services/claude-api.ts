...
class ClaudeApiService {
  ...
  async getOrganizationId(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch organization: ${response.status} ${response.statusText}. Please check your authentication status or try logging in again.`,
      );
    }
    ...
    for (const session of sessions) {
      ...
      if (titleMatch || repoMatch) {
        results.push({
          ...
          repoName,
          matchType:
            titleMatch && repoMatch ? "both" : titleMatch ? "title" : "repo",
        });
      }
    }
